...
func RunBloomSketch(l []disttopk.ItemList, topk int) (disttopk.ItemList, disttopk.AlgoStats) {
	numpeer := len(l)
	N_est := getNEst(l)
	return RunProtocolRunner(tworound.NewBloomPR(topk, numpeer, N_est, 1.0), l)
}

func RunBloomSketchGcs(l []disttopk.ItemList, topk int) (disttopk.ItemList, disttopk.AlgoStats) {
	numpeer := len(l)
	N_est := getNEst(l)
	return RunProtocolRunner(tworound.NewBloomGcsPR(topk, numpeer, N_est, 1.0), l)
}

func RunBloomSketchGcsMerge(l []disttopk.ItemList, topk int) (disttopk.ItemList, disttopk.AlgoStats) {
	numpeer := len(l)
	N_est := getNEst(l)
	return RunProtocolRunner(tworound.NewBloomGcsMergePR(topk, numpeer, N_est, 1.0), l)
}
...
func RunExtraRoundBloomGcsMergeFilter(l []disttopk.ItemList, topk int) (disttopk.ItemList, disttopk.AlgoStats) {
	numpeer := len(l)
	N_est := getNEst(l)
	return RunProtocolRunner(tworound.NewExtraRoundBloomGcsMergePR(topk, numpeer, N_est, 1.0), l)
}
